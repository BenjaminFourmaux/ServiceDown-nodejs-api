...
module.exports = class ServiceDown_api {
	...
	**/
	async getServiceStatus(serviceCN){
		var serviceName = this.getServiceName(serviceCN);
		var statusPath = this.getServicePath(serviceCN);
	...
	**/
	getServicePath(serviceCN) {
		let urlPath = "not available";
		
		// Browse object keys
		Object.keys(servicesList).forEach((key) => {
			if (key == serviceCN.toUpperCase()){
				urlPath = servicesList[key][1];
			}
		});
	...
	**/
	getServiceName(serviceCN) {
		let realName = "unknown";
		
		// Browse object keys
		Object.keys(servicesList).forEach((key) => {
			if (key == serviceCN.toUpperCase()){
				realName = servicesList[key][0];
			}
		});
